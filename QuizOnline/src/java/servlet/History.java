...
public class History extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Vector<Quiz> quizHistory = null;
        User user = (User) session.getAttribute("identify");
        String userId = user.getEmail().trim();
        Page page = new Page();
        if(pageNum != null){
           page.setIndexPage(Integer.parseInt(pageNum));
        }       
        quizHistory = quizS.getQuizHistory(userId,page);
      
        request.setAttribute("page", page);
        request.setAttribute("history", quizHistory);
        request.setAttribute("servlet","History");
